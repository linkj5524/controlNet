...
class ADV_ATTACK:
    def __init__(self, config_path:str='./models/cldm_v15.yaml',
        ...
        if kwargs:
            ...
            self.default_params = {
            "prompt": "a handsome boy with a long hair ,clothes with a red and white striped pattern, and a hat with a red and white striped pattern.",
            "a_prompt": "",
            "n_prompt": "",
            "num_samples": 1,
    ...
    def generate_adversarial_example(self, input_image=None, control_image=None,params=None):
        ...
        latent_input=self.imgTensor_to_latent(temp_tensor,1)
        ...
        for epoch in range(params["optim_epochs"]):
            # 循环，优化
            end_latent=self.ddim_sampler.decode(  latent_start, cond, t_start, unconditional_guidance_scale=2.0, unconditional_conditioning=un_cond,
                use_original_steps=False, callback=None)



            # 转换成图片
            image=self.latent_to_img(end_latent,1)
        ...
        #         use_original_steps=False, callback=None)
        image_tensor=self.latent_to_img(end_latent,1)
    ...
    def generate_adversarial_example1(self, input_image=None,control_image=None, params=None):
        """
        生成对抗样本
    ...
    def generate_adversarial_example_optim_control(self, input_image=None,control_image=None, params=None):
        ...
        latent_start.requires_grad = True
        cond_optim=cond 
        optimizer = torch.optim.Adam(cond_optim["c_crossattn"], lr=5e-1)
        cross_entro_loss = YOLOv11DetectionLoss()
        ...
        for epoch in range(params["optim_epochs"]):
            # 循环，优化
            end_latent=self.ddim_sampler.decode(  latent_start, cond_optim, t_start, unconditional_guidance_scale=2.0, unconditional_conditioning=un_cond,
                use_original_steps=False, callback=None)
    ...
    def generate_adversarial_example_optim_control_v2(self, input_image=None,control_image=None, params=None):
        ...
        cond_optim=cond 
        optimizer = torch.optim.Adam(cond_optim["c_crossattn"], lr=5e-5)
        cross_entro_loss = YOLOv11DetectionLoss()
        ...
        #维度变换
            ...
            img2=cv2.cvtColor(x_samples[0], cv2.COLOR_RGB2BGR)
            cv2.imwrite('result2.png',img2)
            # 转换成图片
            image=self.latent_to_img(samples)
    ...
    def latent_to_img(self,latent,scale=0.18215):
        latent = latent / scale
        img = self.model.first_stage_model.decode(latent)
...
#     #默认输入是RBG格式
#     attack.generate_adversarial_example1(img)
...
if __name__=='__main__':
    ...
    eta = 0.0                     # DDIM采样器的eta参数
    path=os.path.join(os.path.dirname(__file__), "..")
    img_path=os.path.join(os.path.join(path,'test_imgs'),'human.png')
    img=cv2.imread(img_path)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

    control_img=cv2.imread(os.path.join(os.path.join(path,'test_imgs'),'human_line.png'))
    control_img = cv2.cvtColor(control_img, cv2.COLOR_BGR2RGB)
    model_path=os.path.join(os.path.join(path,'models'),'yolo11n.pt')
    attack = ADV_ATTACK(device=torch.device("cuda"),model_path_object_detection=model_path)
    #默认输入是RBG格式
    # attack.generate_adversarial_example1(img,control_img)
