...
class ObjectDetection:
    ...
    def detect(self, img, file_path: Optional[str] = None,file_name='detect.jpg', grad_status: bool = False) :
        """
        支持多batch的YOLO检测主函数
        :param img: 输入图像，支持：
        ...
        """
        if "yolo" not in self.model_type:
            raise ValueError(f"Unsupported model type: {self.model_type}")

        # --------------------------
        # 1. 多batch输入统一处理
        # --------------------------
        img_tensor = self._preprocess_input(img)
...
if __name__ == "__main__":
    ...
    detector = ObjectDetection(
        model_type="yolov11",
        model_path="yolov11n.pt",  # 替换为你的模型路径
        device=device,
        conf_threshold=0.25,
        iou_threshold=0.7
    )
    ...
    batch_imgs_np = np.random.randint(0, 255, size=(2, 640, 640, 3), dtype=np.uint8)
